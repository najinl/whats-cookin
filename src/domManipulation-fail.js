...
import {ingredients, recipeRepository, recipe, user, pantry, targetBtn, displayRecipe, buyIngBtn, missingIngBtn, letsCookBtn, favoritedRecipes, likedRecipes,foundRecipeIngredients, foundRecipeNames} from './scripts'
let welcomename = document.getElementById('username')
let modalContainer, closeModalBtn, markCookedBtn, myListContainer
...
this.addClass(recipeContainer, 'hidden');
this.removeClass(favoritedContainer, 'hidden');
},
...
this.addClass(recipeContainer, 'hidden');
this.addClass(favoritedContainer, 'hidden');
},
...
let modal =
  ...
  this.addClass(modalContainer, 'show')
  // modalContainer.classList.add('show');
  closeModalBtn.addEventListener('click', function() {
  // modalContainer.classList.remove('show');
  this.removeClass(modalContainer, 'show')
  });
  markCookedBtn.addEventListener('click', function() {
    myListContainer.childNodes.forEach(elem => {
      if(elem.id === recipe.id.toString()) {
        ...
        this.addClass(secondChild.children[0], 'hidden')
      // secondChild.children[0].classList.add('hidden')
      this.removeClass(secondChild.children[1], 'hidden')
...
myListContainer.childNodes.forEach(elem => {
  ...
  this.removeClass(secondChild.children[0], 'hidden');
  this.addClass(secondChild.children[1], 'hidden');
